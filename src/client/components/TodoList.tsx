...
export const TodoList = ({ status }) => {
  ...
  const handleClick = (id: number, checked: boolean) => {
    mutation.mutate({ todoId: id, status: checked ? 'completed' : 'pending' })
  }
  ...
  return (
    <ul className="grid grid-cols-1 gap-y-3" ref={parent}>
      {todos.map((todo) => (
        <li key={todo.id}>
          <div
            className={`flex h-[56px] items-center rounded-12 border border-gray-200 px-4 py-3 shadow-sm ${todo.status == 'completed'
          ...
          >
            <Checkbox.Root
              ...
              onCheckedChange={(checked) =>
                handleClick(todo.id, checked === true)
              }
            >
              <Checkbox.Indicator>
